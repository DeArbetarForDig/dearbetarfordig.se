...
const deepChecks: { file: string; sumOf: string[]; shouldEqual: string | null; expectedZero?: boolean; label: string }[] = [
  {
    file: 'budget-grundskola-2026.json',
    sumOf: ['Kulturskola', 'Anpassad grundskola', 'Grundskola inklusive förskoleklass och fritidshem'],
  ...
  },
  {
    ...
    sumOf: ['Personalkostnader', 'Övriga verksamhetskostnader', 'Kapitaltjänst (avskrivningar + ränta)'],
...
for (const check of deepChecks) {
  try {
    ...
    if (diff < TOLERANCE) {
      console.log(`   ✓ ${check.label} (${sumParts.toLocaleString('sv-SE')} ≈ ${expected.toLocaleString('sv-SE')})`)
      deepPassed++
    } else {
      console.log(`   ✗ ${check.label}`)
      console.log(`     Σ delar: ${sumParts.toLocaleString('sv-SE')} ≠ ${check.shouldEqual}: ${expected.toLocaleString('sv-SE')} (diff ${(diff*100).toFixed(1)}%)`)
      deepFailed++
    }
  } catch {
