/**
 * Scraper: goteborg.se nämndhandlingar — KS (Kommunstyrelsen) protokoll och beslut
 *
 * Same approach as handlingar.ts but selects "Kommunstyrelsen" from the dropdown.
 * Usage: npx tsx handlingar-ks.ts [year]
 */
...
async function main() {
  ...
  console.log(`📋 Scraping KS-handlingar ${year}...\n`)
  ...
  try {
    ...
    const outPath = join(OUTPUT_DIR, `ks-handlingar-${year}.json`)
