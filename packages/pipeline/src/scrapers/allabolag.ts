...
async function main() {
  ...
  const limit = Number.parseInt(process.argv[2] || '5') // Default: first 5 for testing
  ...
  for (let i = 0; i < Math.min(limit, polData.politiker.length); i++) {
    ...
    try {
      await page.goto(`https://www.allabolag.se/befattningshavare?q=${encodeURIComponent(name)}`, {
      ...
      const bolagsuppdrag = await page.evaluate(() => {
        ...
        if (roles.length === 0) {
          document.querySelectorAll('[class*="Role"], [class*="role"]').forEach((el) => {
            ...
            if (
              text.includes('Ledamot') ||
              text.includes('Ordförande') ||
              text.includes('VD') ||
              text.includes('Suppleant')
            ) {
              const link = el.querySelector('a[href*="/foretag/"]')
              roles.push({
                roll: text.split('\n')[0],
                bolag: link?.textContent?.trim() || text,
  ...
  writeFileSync(
    outPath,
    JSON.stringify(
      { hämtad: new Date().toISOString(), antal: results.length, politiker: results },
      null,
      2,
    ),
  )
