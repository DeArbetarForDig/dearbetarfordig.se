...
export const politiker = goteborg.table(
  'politiker',
  {
  ...
  (t) => [index('idx_politiker_parti').on(t.parti)],
)

export const grafNodes = goteborg.table(
  'graf_nodes',
  {
  ...
  (t) => [index('idx_graf_nodes_typ').on(t.typ)],
)

export const dokument = goteborg.table(
  'dokument',
  {
    id: text('id').primaryKey(),
...
export const grafEdges = goteborg.table(
  'graf_edges',
  {
    ...
    fromId: text('from_id')
      .notNull()
      .references(() => grafNodes.id, { onDelete: 'cascade' }),
    toId: text('to_id')
      .notNull()
      .references(() => grafNodes.id, { onDelete: 'cascade' }),
    typ: text('typ').notNull(),
